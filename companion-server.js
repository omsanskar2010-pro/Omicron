...
const ACTIONS = {
  ...
  open_notepad:       () => runShell(`start notepad`),
  open_url:           (params) => openUrl(params),
...
function openUrl(params) {
  ...
  // Only allow http/https — anything else (file:, javascript:, etc.) is rejected
  // to stop this from being used to run arbitrary local commands.
  if (!/^https?:\/\//i.test(url)) {
    return Promise.resolve({ ok: false, error: "Only http:// or https:// URLs are allowed." });
  }
  return runShell(`start "" "${url}"`);
}
